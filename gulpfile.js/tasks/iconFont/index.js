...
var settings = {
  ...
  dest: path.join(config.root.dest, config.tasks.iconFont.dest, '/icons'),
  sassDest: path.join(config.root.assetsSrc, config.tasks.css.src, config.tasks.iconFont.sassDest),
  template: path.normalize('./gulpfile.js/tasks/iconFont/template.scss'),
  sassOutputName: '_icons.scss',
  fontPath: path.join('..', '..', '..', '..', '..', 'public', 'fonts', 'icons'),
  className: 'icon',
  options: {
    svg: true,
    timestamp: 0,
